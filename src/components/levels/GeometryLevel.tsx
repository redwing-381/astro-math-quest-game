...
export const GeometryLevel: React.FC<GeometryLevelProps> = ({
  onBack,
  onComplete
}) => {
  ...
  const storyStages = ["🛰️ Emergency! The satellite's main solar panel is damaged and needs repair...", "🔧 Good progress! But more components need fixing to restore full power!", "⚡ Almost operational! One final repair will bring the satellite back online!", "🌟 Mission accomplished! The satellite is fully operational and transmitting!"];
  ...
  const TriangleVisual = () => {
    ...
    return <div className="flex justify-center">
        <svg width="300" height="200" className="border border-white/30 rounded bg-slate-800/30">
          {/* Grid lines */}
          {[...Array(21)].map((_, i) => <g key={i}>
              <line x1={i * 15} y1="0" x2={i * 15} y2="200" stroke="rgba(255,255,255,0.1)" strokeWidth="1" />
              <line x1="0" y1={i * 10} x2="300" y2={i * 10} stroke="rgba(255,255,255,0.1)" strokeWidth="1" />
            </g>)}
          
          {/* Triangle */}
          <polygon points={`50,150 ${50 + problem.base * scale},150 ${50 + problem.base * scale / 2},${150 - problem.height * scale}`} fill="rgba(34, 197, 94, 0.3)" stroke="rgb(34, 197, 94)" strokeWidth="3" />
          
          {/* Base line */}
          <line x1="50" y1="150" x2={50 + problem.base * scale} y2="150" stroke="rgb(59, 130, 246)" strokeWidth="3" />
          
          {/* Height line */}
          <line x1={50 + problem.base * scale / 2} y1="150" x2={50 + problem.base * scale / 2} y2={150 - problem.height * scale} stroke="rgb(239, 68, 68)" strokeWidth="3" strokeDasharray="5,5" />
          ...
          <Card className="bg-white/10 backdrop-blur-sm border-white/20">
            <div className="p-6">
              ...
              <div className="mb-8 text-center">
                <div className={`w-24 h-24 mx-auto rounded-full border-4 flex items-center justify-center transition-all duration-1000 ${repairProgress === 100 ? 'border-green-400 bg-green-400/20 animate-pulse' : 'border-red-400 bg-red-400/20'}`}>
              ...
              <div className="space-y-4">
                <div className="text-center">
                  ...
                  <Input type="number" step="0.5" placeholder="Enter area" value={userAnswer} onChange={e => setUserAnswer(e.target.value)} className="w-32 mx-auto text-center bg-white/20 border-white/30 text-white text-xl" />
                </div>

                <Button onClick={checkAnswer} className="w-full bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white font-bold py-3" disabled={!userAnswer}>
